...
class ApiController extends Controller
{
  ...
  public function getAutoCompleteObat(Request $req){
		$data = \DB::select('select obat.id as data,obat.nama as value, satuan.nama as satuan from obat inner join satuan on obat.satuan_kecil_id = satuan.id where obat.nama like "%'.$req->get('nama').'%"');
	...
	public function getAutoCompleteSupplier(Request $req){
		$data = \DB::select('select id as data,nama as value from supplier where nama like "%'.$req->get('nama').'%"');
		$data_res = ['query'=>'Unit','suggestions' => $data];
